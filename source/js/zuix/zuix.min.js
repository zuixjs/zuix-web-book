...
0<=f[0].indexOf("\n")))f=a.substr(l,f.index-l)+f[0]+"}",e+=f,l+=f.length;else{var h="{"+f[0]+"}";if("function"===typeof d){var m=d(f[0]);n.isNoU(m)||(h=m,g++)}e+=a.substr(l,f.index-l-1)+h;l=f.index+f[0].length+1}return 0<g?e+=a.substr(l):null};h.getClosest=function(a,d){for(;a&&a!==document;a=a.parentNode)if(a.matches(d))return a;return null};h.getPosition=function(a,d){for(var b=0,e=0,g=a.getBoundingClientRect(),f=a;f;){if("body"===f.tagName.toLowerCase())var m=f.scrollTop||document.documentElement.scrollTop,
b=b+(f.offsetLeft-(f.scrollLeft||document.documentElement.scrollLeft)+f.clientLeft),e=e+(f.offsetTop-m+f.clientTop);else b+=f.offsetLeft-f.scrollLeft+f.clientLeft,e+=f.offsetTop-f.scrollTop+f.clientTop;f=f.offsetParent}b={x:b,y:e,rect:g,visible:!1};g=a.offsetParent;if(null!=g){g.getBoundingClientRect();if(g===document.body)e=document.body.offsetWidth,g=document.body.offsetHeight,e=document.documentElement.clientWidth||e,m=document.documentElement.clientHeight||g;else{for(f=g.offsetParent;null!=f&&
null!==f.offsetParent&&f.offsetHeight===f.scrollHeight;)f=f.offsetParent;null!=f?(e=f.offsetWidth,m=f.offsetHeight):(e=g.offsetWidth,m=g.offsetHeight)}null==d&&(d=0);var g=e,f=m,n=a.getBoundingClientRect(),g=!(n.left>g-d||n.right<0+d||n.top>f-d||n.bottom<0+d);b.visible=g;b.frame={dx:(n.left+n.width/2-0)/e,dy:(n.top+n.height/2-0)/m};a=h(a);!g&&a.hasClass("--ui--visible")?(a.removeClass("--ui--visible"),b.event="exit"):g?g&&(a.hasClass("--ui--visible")?b.event="scroll":(b.event="enter",a.addClass("--ui--visible"))):
